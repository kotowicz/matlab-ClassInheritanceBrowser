classdef ClassHelpContainer < helpUtils.containers.abstractHelpContainer
    ...
    methods
        ...
        %% ---------------------------------
        function b = containsEvents(this)
            % CONTAINSEVENTS - returns true if this class contains any
            % events.
            b = ~isempty(this.SimpleElementHelpContainers.('events'));
        end
...
function memberHelpContainer = getMemberHelpContainer(memberStruct, memberName)
    ...
    if isfield(memberStruct, memberName)
        ...
        error(message('MATLAB:ClassHelpContainer:UndefinedClassMember', mat2str( memberName )));
    end
